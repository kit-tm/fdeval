    ...
    def on_EVSwitchStats(self, switch, ev):
        ...
        if switch.label == 'DS':
            if math.isclose(ev.tick, 7):
                print("@%.0f remove for ds1" % ev.tick)
                for id, flow in self.active_flows.items():
                    self.remove_delegation(ev.tick, flow, switch, es)

        if switch.label == 'DS2':
            if math.isclose(ev.tick, 9):
                print("@%.0f remove for ds2" % ev.tick)
                for id, flow in self.active_flows.items():
                    self.remove_delegation(ev.tick, flow, switch, es)

        if switch.label == 'DS':
            if math.isclose(ev.tick, 11):
                print("@%.0f add for ds" % ev.tick)
                for id, flow in self.active_flows.items():
                    self.add_delegation(ev.tick, flow, switch, es)
...
class MyTopo( Topo ):
    ...
    def __init__( self, ctx ):
        ...
        self.addLink( ds, ds2, capacity=1000, propagation_delay=propagation_delay )
        ...
        # add traffic
        self.addTraffic(            
            dict(fg_class='Single', fg_label="f0", fg_start=0,  fg_demand=150, fg_duration=15, 
            fg_fixed_path=['h1', 'DS', 'DS2', 'h2']))
...
FLOW_TIMINGS = """{"DS->ES": {"f0": [3, 16.0]}, "DS->DS2": {"f0": [0.5, 17.0]}, "DS->h1": {}, 
"DS2->DS": {}, "DS2->ES": {"f0": [5, 9]}, "DS2->h2": {"f0": [1.0, 17.5]}, "ES->DS": 
{"f0": [3.0, 16.5]}, "ES->DS2": {"f0": [5.0, 9.5]}, "h1->DS": {"f0": [0, 15.5]}, "h2->DS2": {}}"""