...
#from .mn.util import irange, natural, naturalSeq
# @bauer: this file used three simple functions from mn.util; because
# all the other stuff from mn.util is not required, these three functions were 
# copied here
...
class Topo( object ):
    ...
    def __init__( self, *args, **params ):
        ...
        self.ctx = None
        self.traffic = params.pop( 'traffic', [] )
    ...
    def paramInt(self, name, default = 0):
        if not self.ctx.config.get(name):
            self.ctx.config[name] = int(default)
        else:
            self.ctx.config[name] = int(self.ctx.config[name])
        return self.ctx.config[name]

    def addTraffic( self, *args, **opts ):
        """added by @robert.bauer@kit.edu
            Define traffic for the simulator;"""
        for flow_spec in args:
            self.traffic.append(flow_spec)

    def addFlow( self, **opts ):
        """added by @robert.bauer@kit.edu
            Define traffic for the simulator;"""
        self.traffic.append(opts)
